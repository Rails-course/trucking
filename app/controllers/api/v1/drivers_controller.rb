  ...
  module V1
    class DriversController < ApiController
      def index
        ...
        drivers = paginate_collection(User.select(drivers_api_columns).where(role: Role.find_by(role_name: 'driver')))[0]
        render json: { drivers: drivers.to_json(include: [company: { only: :name }]),
                       drivers_count: total_count(User.select(drivers_api_columns).where(role: Role.find_by(role_name: 'driver'))) }
      end
    end
  end
end
